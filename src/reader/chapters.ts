...
export function markNoise(blocks: Block[]): Block[] {
  ...
  for (const [page,indices] of perPage) {
    for (const index of new Set([...indices.slice(0,2),...indices.slice(-2)])) {
      ...
      if (text.length > 200 || text.length < 5 || /^(cap[ií]tulo|parte|secci[oó]n)\b/i.test(text)) continue;
...
function isHeading(block: Block, i: number, blocks: Block[]) {
  ...
  if (block.kind === 'noise' || text.length > 150 || text.length < 3 || /\.{2,}|\s\d+\s*$/.test(text) && !/^(cap[ií]tulo|parte|secci[oó]n)\s+\d+$/i.test(text)) return false;
  if (chapterKind(text) !== 'content') return true;
  if (/^(pr[oó]logo|prefacio|introducci[oó]n|ep[ií]logo|conclusi[oó]n|conclusiones)$/i.test(text)) return true;
  if (/^(cap[ií]tulo|parte|secci[oó]n)\s+(\d+|[ivxlcdm]+|primero|segundo|tercero|cuarto|quinto)(?:\b|\s|[.:—-])/i.test(text)) return true;
  ...
  return letters.length >= 8 && letters === letters.toUpperCase() && !/[.!?]$/.test(text) && pageStart && (blocks[i+1]?.text.length ?? 0) > 150;
}
