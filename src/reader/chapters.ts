...
export function markNoise(blocks: Block[]): Block[] {
  ...
  for (const [page,indices] of perPage) {
    for (const index of new Set([...indices.slice(0,2),...indices.slice(-2)])) {
      ...
      if (text.length > 200 || text.length < 5 || /^(capitulo|parte|seccion)\b/.test(key(text))) continue;
...
function isHeading(block: Block, i: number, blocks: Block[]) {
  ...
  if (text.length > 150 || !text || /\.{2,}/.test(text)) return false;
  ...
  const titleCase = words.length >= 2 && words.length <= 12 && text.length <= 90 && /^[\p{Lu}][\p{L}]*(?:\s+[\p{Lu}][\p{L}]*|\s+(?:de|del|la|el|los|las|y|a|en|du|des|le|les|et))+$/u.test(text);
